...
# This dataset contains information about different houses and their prices
housing = fetch_california_housing()
X = housing.data       # type: ignore # Features (input variables)
y = housing.target     # type: ignore # Target (house prices)
# Step 3: Create a DataFrame for easier manipulation
df = pd.DataFrame(X, columns=housing.feature_names) # type: ignore
df['Price'] = y
...
rmse_lin = mean_squared_error(y_test, y_pred_lin, squared=False)  # Calculate RMSE for Linear Regression

# Decision Tree Evaluation
mae_tree = mean_absolute_error(y_test, y_pred_tree)  # Calculate MAE for Decision Tree
rmse_tree = mean_squared_error(y_test, y_pred_tree, squared=False)  # Calculate RMSE for Decision Tree
